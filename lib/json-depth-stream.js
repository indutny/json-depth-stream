...
class StreamIndexer extends Writable {
  constructor(depth) {
    super();
  ...
  _enter(state, offset) {
    this.backlog.push({ state: this.state, offset });
    this.state = state;
    return state;
  }
  ...
  _visit(offset, chunk) {
    ...
    this.emit('visit', this.path.slice(), last.offset + 1, offset, chunk);
  }
  ...
  update(chunk, start, end) {
    ...
    this.emit('_chunkStart', chunk, start || 0, limit);
  ...
  query(target) {
    // TODO(indutny): throw on too deep target
    const q = new Query(target);

    this.on('_chunkStart', (chunk, start, end) => {
      q._chunkStart(chunk, start, end);
    });
